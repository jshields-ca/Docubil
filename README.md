...
The frontend lives in `client/` (React + Vite + Tailwind CSS). For hot-reload
during UI development, run it alongside the backend in a second terminal —
...
[`client/README.md`](client/README.md) for details.
...
Builds the frontend straight into `public/`, which Express serves as static
files. This runs automatically as part of the Docker image build.
...
│   └── src/
├── public/                   # Built frontend output (generated by `npm run build`, not tracked in git)
├── src/
